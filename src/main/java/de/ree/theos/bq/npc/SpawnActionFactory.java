...
public record SpawnActionFactory(NpcManager npcManager) implements PlayerActionFactory, PlayerlessActionFactory {
    ...
    private NullableActionAdapter parse(final Instruction instruction) throws QuestException {
        ...
        return new NullableActionAdapter(profile
                -> npcManager.get(profile, identifier.getValue(profile)).spawn(location.getValue(profile)));
    }
}
